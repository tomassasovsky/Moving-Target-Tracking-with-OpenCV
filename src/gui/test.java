...
public class test {
    ...
    public void playVideo() {
        ...
        while (camera.read(inFrame)) {
            ...
            if (i == 0) {
                ...
                new Mat(outbox.size(), CvType.CV_8UC1);
                outbox.clone();
            }
            ...
            if (i == 1) {
                ...
                if (array.size() > 0) {
                    tracker.update(array, detections, image);
                    for (Rect obj : array) {
                        int ObjectCenterX = (int) ((obj.tl().x + obj.br().x) / 2);
                        int ObjectCenterY = (int) ((obj.tl().y + obj.br().y) / 2);
                        ...
                        Imgproc.rectangle(image, obj.br(), obj.tl(), new Scalar(
                                0, 255, 0), 2);
                        Imgproc.circle(image, pt, 1, new Scalar(0, 0, 255), 2);
                    }
                } else {
                    tracker.updateKalman(image, detections);
                }
                ...
                Imgproc.putText(image, "Input: " + CONFIG.filename, new Point(20, 360),
                        Imgproc.FONT_HERSHEY_PLAIN, 1, new Scalar(255, 255, 255),
                        1);
                Imgproc.putText(image,
                        "So track hien tai: " + tracker.tracks.size()
                                + "     Da xoa: " + tracker.track_removed,
                        new Point(20, 50), Imgproc.FONT_HERSHEY_PLAIN, 1,
                        new Scalar(255, 255, 255), 1);
            }

            i = 1;

            ImageIcon image = new ImageIcon(Functions.Mat2bufferedImage(test.image));
    ...
    public void initialize() {
        ...
        btnOrgin.addActionListener(e -> option = 1);

        JButton buttonBG = new JButton("BS");
        buttonBG.addActionListener(e -> option = 2);
        buttonBG.setBounds(415, 421, 89, 29);
        jFrame.getContentPane().add(buttonBG);

        JButton buttonResult = new JButton("Result");
        buttonResult.setBackground(Color.PINK);
        buttonResult.addActionListener(e -> option = 0);
        buttonResult.setBounds(514, 421, 89, 29);
        jFrame.getContentPane().add(buttonResult);
        ...
        buttonOpen.addActionListener(arg0 -> {
        ...
        btnCaptureCamera.addActionListener(arg0 -> {
            // TODO Auto-generated method stub
            inputCapture = true;
            playVideo();
            jFrame.repaint();
